...
class Point_1600 {
    ...
    Point_1600(int x, int y, int k, int count) {
        this.x = x;
        this.y = y;
        this.k = k;
        this.count = count;
...
public class Main_1600_말이되고픈원숭이 {
    ...
    private static Queue<Point_1600> queue = new LinkedList<>();
    ...
    private static int bfs() {
        ...
        queue.offer(new Point_1600(0, 0, K, 0));
        visited[0][0][0] = true;
        while (!queue.isEmpty()) {
            Point_1600 cur = queue.poll();
    ...
    private static void move(int[] dx, int[] dy, Point_1600 cur, int nk) {
        for (int i = 0; i < dx.length; i++) {
            int nx = cur.x + dx[i];
            int ny = cur.y + dy[i];
            if (nx < 0 || nx >= H || ny < 0 || ny >= W) continue;
            if (visited[nx][ny][nk] || area[nx][ny] == 1) continue;
            queue.offer(new Point_1600(nx, ny, nk, cur.count + 1));
